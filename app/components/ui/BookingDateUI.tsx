...
export const BookingDateUI = ({ formData, errors, availableDates = [], isLoading, onDateSelect }: BookingDateUIProps) => {
  ...
  const selectedDate = formData.date 
    ? parseDate(format(typeof formData.date === "string" ? parseISO(formData.date) : new Date(formData.date), "yyyy-MM-dd"))
    : null;
  ...
  const isDateUnavailable = (date: CalendarDate) => {
    ...
    return (availableDates || [])
      ...
      .some((disabledDate) => 
        format(new Date(disabledDate.date), "yyyy-MM-dd") === format(jsDate, "yyyy-MM-dd")
      );
  };
          ...
          <Calendar
            ...
            classNames={{
              ...
              cellButton: cn(
                ...
                "aria-unavailable:line-through aria-unavailable:text-red-500"
              ),
              headerWrapper: "mb-4",
